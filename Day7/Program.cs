...
namespace Day7
{
    class Program
    {
        ...
        public static int Exercise1(int[] input)
        {
            Array.Sort(input);// Sorting the values in asc order
            int min = input[0];
            int max = input[input.Length - 1];
            int currentValue = min;
            int previousSum = max * (input.Length - 1); // Highest value possible is the last value times the size of the list -1 (table full of 0 and a distance of the highest value stored in last cell)
            int buffer = max * (input.Length - 1); // buffer is set to the max value as well before the first iteration
            do
            {
                previousSum = buffer;
                buffer = 0;
                foreach (int crab in input)
                { // we calculate the distance for every element in the table
                    buffer += Math.Abs(crab - currentValue);
                }
                currentValue++;
            } while (buffer <= previousSum && currentValue <= max); // the distance is supposedly a U shape with a bottom somwhere around the mean. I might be totally wrong

            return previousSum;
        }
        public static long Exercise2(int[] input)
        {
        ...
        public static long sumInt(int a, int b) // Function to calculate the sum from 1 to n where n is the distance between a and b 
        {
            ...
            if (distance % 2 == 0)
            {
                finalSum = (long)(distance * (distance / 2) + mean);
                return finalSum;
            }
            else
            {
                finalSum = (long)(distance * meanUpped);
                return finalSum;
            }
        }
    }
