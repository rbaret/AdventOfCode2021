...
namespace Day10
{
    class Program
    {
        ...
        public static long Exercise1(List<string> input)
        {
            ...
            List<char> leftBrackets = new List<char>() { '(', '{', '<', '[' }; // Allowed left brackets to be added to the stack
            ...
            {
                int index = 0;
                bracketsStack.Clear(); // Start with an empty stack
                illegalFound = false;
                do
                {

                    char current = line[index];
                    if (bracketsStack.Count == 0) // The list is empty, only a left bracket is allowed
                    {
                    ...
                    { // The list is not empty, we can only add left brackets
                        if (leftBrackets.Contains(current)) // We have a left bracket
                            bracketsStack.Push(current); // W add it to the list
                        else // We have a right bracket
                        {
                            if (correspondance[bracketsStack.Peek()] == current) // This character is the corresponding char for the last item of the pile
                            {
                                bracketsStack.Pop();
                            }
            ...
            foreach (string line in input)
            {
                ...
                do
                {
                    ...
                    if (bracketsStack.Count == 0) // The list is empty, only a left bracket is allowed
                    {
                        ...
                        {
                            bracketsStack.Push(current); // If so, add it to the stack
                        }
                        else
                        {
                    ...
                    {
                        if (leftBrackets.Contains(current)) // We have a left bracket
                            bracketsStack.Push(current); // We add it to the list
                        else // We have a right bracket
                        {
                            if (correspondance[bracketsStack.Peek()] == current) // If this character is the corresponding char for the last item of the pile
                            {
                                bracketsStack.Pop(); // We remove the closed bracket from the stack
                            }
                            else // ILLEGAL POLICE ALERT AAAAHHH
                            {
                ...
                if (!illegalFound && bracketsStack.Count > 0) // We have finished to read the line, time to check for icomplete lines
                {
                    do
                    {
                        currentScore = currentScore * 5 + bracketScores[bracketsStack.Pop()]; // score calculation based on the rules
                    } while (bracketsStack.Count > 0); // Do it until the stack is empty
